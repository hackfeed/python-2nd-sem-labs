...
def draw_plot(dim_entry, step_entry, eps_entry, maxiter_entry):
    ...
    for i in range(1, 49999):
        if fx[i] >= round(max_graph, 6):
            max_graph = round(fx[i], 6)
            max_graph_dot = x[i]
        if fx[i] <= round(min_graph, 6):
            min_graph = round(fx[i], 6)
            min_graph_dot = x[i]
...
about_menu.add_command(label="Коды ошибок", command=lambda: get_code_info())
